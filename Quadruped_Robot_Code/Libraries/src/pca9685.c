 ...
 *  open drain or push pull / totem pole.
 *  Warning: LEDs with integrated zener diodes should
 *  only be driven in open drain mode.
 *  default is totem pole
 *  @return  0 : Task done without error.
 * 	1 : Error occurred.
 */
PCA9685_STATUS PCA9685_SetOutputMode(bool totempole)
{
	uint8_t oldmode, newmode;
	if(HAL_I2C_Mem_Read(pca9685_i2c, PCA9685_ADDRESS, PCA9685_MODE2, 1, &oldmode, 1, 10)) return PCA9685_ERROR;
  if(totempole)newmode = oldmode | MODE2_OUTDRV;
  else newmode = oldmode & ~MODE2_OUTDRV;
	if(HAL_I2C_Mem_Write(pca9685_i2c, PCA9685_ADDRESS, PCA9685_MODE2, 1, &newmode, 1, 10)) return PCA9685_ERROR;
	return PCA9685_OK;
}

/*!
 *  @brief  Gets the output mode of the PCA9685 of either
 *  open drain or push pull / totem pole.
 * @return	0 if the outputs are configured with an open-drain structure.
 *  1 : The outputs are configured with a totem pole structure.
 */
PCA9685_STATUS PCA9685_GetOutputMode()
{
	uint8_t data;
	HAL_I2C_Mem_Read(pca9685_i2c, PCA9685_ADDRESS, PCA9685_MODE2, 1, &data, 1, 10);
	return (data & MODE2_OUTDRV)?1:0;
}
#endif
